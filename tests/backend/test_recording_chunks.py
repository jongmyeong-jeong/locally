...
@pytest.fixture
def doc_id(conn):
    """Insert a throwaway document row and clean up recording_chunks after."""
    doc = db.create_document(conn, title="test-doc")
    did = doc["id"]
    yield did
    conn.execute("DELETE FROM recording_chunks WHERE document_id = ?", (did,))
...
class TestInsertChunk:
    def test_insert_chunk_creates_queued_row(self, conn, doc_id):
        """insert_chunk → status='queued', retry_count=0, text=None."""
        chunk_id = recording_chunks.insert_chunk(conn, doc_id, seq=0, start_ms=0, end_ms=5000)
        rows = recording_chunks.get_chunks(conn, doc_id)
        assert len(rows) == 1
        row = rows[0]
...
class TestUpdateChunkStatus:
    def test_update_chunk_status_transitions_to_success(self, conn, doc_id):
        """update_chunk_status → 'success' with text='hello'."""
        chunk_id = recording_chunks.insert_chunk(conn, doc_id, seq=0, start_ms=0, end_ms=3000)
        recording_chunks.update_chunk_status(conn, chunk_id, "success", "hello")
        rows = recording_chunks.get_chunks(conn, doc_id)
        assert rows[0]["status"] == "success"
        assert rows[0]["text"] == "hello"

    def test_update_chunk_status_to_failed(self, conn, doc_id):
        """update_chunk_status → 'failed' → get_failed_chunks returns the row."""
        chunk_id = recording_chunks.insert_chunk(conn, doc_id, seq=0, start_ms=0, end_ms=3000)
        recording_chunks.update_chunk_status(conn, chunk_id, "failed")
        failed = recording_chunks.get_failed_chunks(conn, doc_id)
        assert len(failed) == 1
...
class TestAllChunksDone:
    def test_all_chunks_done_only_when_all_success(self, conn, doc_id):
        """all_chunks_done → True only when all rows are 'success'; False for any non-success."""
        ids = [
            recording_chunks.insert_chunk(conn, doc_id, seq=i, start_ms=i * 5000, end_ms=(i + 1) * 5000)
            for i in range(3)
        ]
        # Initially all queued → False.
        assert recording_chunks.all_chunks_done(conn, doc_id) is False
        ...
        assert recording_chunks.all_chunks_done(conn, doc_id) is True

    def test_all_chunks_done_returns_false_for_zero_chunks(self, conn, doc_id):
        """all_chunks_done returns False when no rows exist (empty doc)."""
        assert recording_chunks.all_chunks_done(conn, doc_id) is False


class TestGetChunksOrdering:
    def test_get_chunks_orders_by_seq(self, conn, doc_id):
        """get_chunks returns rows ordered by seq regardless of insertion order."""
        recording_chunks.insert_chunk(conn, doc_id, seq=2, start_ms=10000, end_ms=15000)
        recording_chunks.insert_chunk(conn, doc_id, seq=0, start_ms=0, end_ms=5000)
        recording_chunks.insert_chunk(conn, doc_id, seq=1, start_ms=5000, end_ms=10000)
        rows = recording_chunks.get_chunks(conn, doc_id)
        assert [r["seq"] for r in rows] == [0, 1, 2]


class TestInvalidStatus:
    def test_invalid_status_raises(self, conn, doc_id):
        """update_chunk_status with an unrecognised status raises ValueError."""
        chunk_id = recording_chunks.insert_chunk(conn, doc_id, seq=0, start_ms=0, end_ms=1000)
...
class TestSessionTranscribeQueueRetrySemantics:
    def test_session_transcribe_queue_failed_after_two_attempts(self, tmp_home, monkeypatch):
        ...
        conn = db.open_db()
        doc = db.create_document(conn, title="retry-test")
        did = doc["id"]
        ...
        async def _run():
            ...
            await q.push(transcribe_queue.ChunkJob(
                chunk_id=id0, document_id=did, seq=0,
                start_ms=0, end_ms=5000, audio_path=str(audio0),
            ))
            await q.push(transcribe_queue.ChunkJob(
                chunk_id=id1, document_id=did, seq=1,
